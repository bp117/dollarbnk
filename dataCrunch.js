...
export function constructServiceDepPath(rangeData, serviceName, traceColors={}, countTracePredicate=()=>true) {
    ...
    Object.keys(rangeData).forEach(traceId=>{
        ...
        if(serviceTransId){
            ...
            backtrace.nodes.forEach(node=>{
                if(countTracePredicate(node, traceId)){
                    nodesTraceCount[node["service.name"]] = [...(nodesTraceCount[node["service.name"]]||[]), traceId];
                    nodesTraceCount[node["service.name"]] = [...new Set(nodesTraceCount[node["service.name"]])];  //Remove duplicate trace ids.
                }
                if(node.isTarget){
