...
export default class D3GraphManager {
    ...
    initTraceListModal = ()=>{
        ...
        div.innerHTML = `
            <div id="modal-${this.svgId}" class="modal">

                <!-- Modal content -->
                <div class="modal-content">
                    <div class="modal-header">
                        <h2 id="traces-list-title" style="display:inline-block; margin-top:5px">Traces</h2>
                        <span class="close-btn" id="close-btn-${this.svgId}">&times;</span>
                    </div>
                    <div class="modal-body">
                        <div class="item-traces-list" id="modal-content-${this.svgId}"></div>
                    </div>
                </div>

            </div>
        `;
        this.anchorNode.appendChild(div);
    }
            ...
            if(this.gType == "ANALYZER"){
                ...
                this.dagre.setNode(getServiceId(item), {
                    ...
                    class: (item.traces?"["+traces.join(";")+"] ":"") + 
                            (`analyzer-node node-graph-id=${this.svgId} `) + 
    ...
    drawGraph = (hideSpans) => {
        ...
        setTimeout(() => {
            ...
            document.querySelectorAll("g.node.analyzer-node").forEach(item => {
                ...
                let nodeIdType = classList.find(t=>t.startsWith("node-graph-id="));
                
                if(nodeIdType.substring(nodeIdType.indexOf("=")+1)!==this.svgId) return;

                let tracesStr = classList.find(t=>t.startsWith("[")&&t.endsWith("]")) || "";
                ...
                item.onclick = ()=>{
                    ...
                    let modal = document.getElementById(`modal-${this.svgId}`);
                    modal.style.display = "block";

                    let modalContent = document.getElementById(`modal-content-${this.svgId}`);
                    modalContent.innerHTML = traces.map(trace=>(`
                        <div class="traces-item" style="color:${trace[1]}">
                            ...
                            <div class="copy-item" data-trace="${trace[0]}"><span class="fa fa-copy" style="font-size:16px"></span></div>
                        </div>
                    `)).join("");
                    ...
                    const removeModal = ()=>{
                        ...
                        let span = document.getElementById(`close-btn-${this.svgId}`);
